...
//    }
    ...
    private Flow searchAirlineFlow() throws Exception {
    	Flow adios = airlineFlow("adios", requestAdiosStep());
    	Flow flyUs = airlineFlow("flyUs", requestFlyUsStep());
    	Flow sp = airlineFlow("sp", requestSouthPacificStep());
    	Flow ta = airlineFlow("ta", requestTransamericanStep());
    	Flow be = airlineFlow("be", requestBelarusStep());
    	Flow oc = airlineFlow("oc", requestOceanicStep());
    	
    	return new FlowBuilder<SimpleFlow>("parallelSearchFlow")
    			.split(taskExecutor())
    			.add(adios, flyUs, sp, ta, be, oc)
    			.build();
    }
